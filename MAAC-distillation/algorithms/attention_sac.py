...
class AttentionSAC(object):
    ...
    def update_policies(self, sample,alpha,base_model, soft=True, logger=None, **kwargs):
        obs, acs, rews, next_obs, dones = sample
        samp_acs = []
        all_probs = []
        all_log_pis = []
        ...
        for a_i, probs, log_pi, pol_regs, (q, all_q) in zip(range(self.nagents), all_probs,
            ...
            # 以上为Q值
            #print(pol_loss)
            model_out = self.target_policies[a_i](next_obs[a_i], model_original_out=True)
            # print(1)
            #print(model_out)
            # print(model_out)

            model_logits = get_logits(base_model[a_i], next_obs[a_i])
            # # print(2)
            #print(model_logits)
            a = self_softmax(model_out)
            #a = F.log_softmax(model_out,dim=-1)
            b = self_softmax(torch.div(model_logits,10))
            #print(a)
            #print(b)

            #cross_loss = torch.nn.L1Loss()(model_out, model_logits)#300-280-1700
            #cross_loss = torch.nn.CrossEntropyLoss()(a,b.long())

            cross_loss = torch.nn.MSELoss()(model_out, model_logits)

            #print(cross_loss)

            #cross_loss = torch.nn.KLDivLoss()(a,b)

            #cross_loss = scipy.stats.entropy(a, b)
            #cross_loss =  softmax_cross_entropy_loss(a,b)

            #print(cross_loss)

            # 要对loss进行缩放，保持在同一个范围、
            # pol_loss = pol_loss * 1000
            # cross_loss = cross_loss * 10
            #print(pol_loss)
            #print(cross_loss)

            loss_all = (1 - alpha) * pol_loss  + alpha * 0.01* cross_loss
            ...
            if logger is not None:
                ...
                logger.add_scalars('agent%i/losses' % a_i,
                                   {'cross_loss': cross_loss,
                                    'pol_loss': pol_loss,
                                    'loss_all': loss_all},
                                   self.niter)

                logger.add_scalars('component',
                                   {'alpha': alpha,
                                    'loss_all': loss_all},
                                   self.niter)
    ...
    def save2(self, dir,episode):
        """
        Save trained parameters of all agents into one file
        """

        dir = os.path.join(dir, '{}'.format(episode))
        if not os.path.exists(dir):  # make the path
            os.mkdir(dir)
        self.prep_training(device='cpu')  # move parameters to CPU before saving\
        for i, a in enumerate(self.agents):
            torch.save(a.policy, os.path.join(dir, 'a_c_{}.pt'.format(i)),_use_new_zipfile_serialization=(False))
        # save_dict = {'init_dict': self.init_dict,
        #              'critic_params': {'critic': self.critic.state_dict(),
        #                                'target_critic': self.target_critic.state_dict(),
        #                                'critic_optimizer': self.critic_optimizer.state_dict()}}
